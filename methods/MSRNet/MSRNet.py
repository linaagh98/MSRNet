...
from .layers import ABSIU, MGFU, SimpleASPP
from .ops import ConvBNReLU, PixelNormalizer, resize_to

LOGGER = logging.getLogger("main")


class _MSRNet_Base(nn.Module):
    @staticmethod
...
class RN50_MSRNet(_MSRNet_Base):
    def __init__(
        ...
        self.siu_5 = ABSIU(mid_dim, siu_groups)
        self.hmu_5 = MGFU(mid_dim, hmu_groups, num_frames=num_frames)

        self.tra_4 = ConvBNReLU(1024, mid_dim, 3, 1, 1)
        self.siu_4 = ABSIU(mid_dim, siu_groups)
        self.hmu_4 = MGFU(mid_dim, hmu_groups, num_frames=num_frames)

        self.tra_3 = ConvBNReLU(512, mid_dim, 3, 1, 1)
        self.siu_3 = ABSIU(mid_dim, siu_groups)
        self.hmu_3 = MGFU(mid_dim, hmu_groups, num_frames=num_frames)

        self.tra_2 = ConvBNReLU(256, mid_dim, 3, 1, 1)
        self.siu_2 = ABSIU(mid_dim, siu_groups)
        self.hmu_2 = MGFU(mid_dim, hmu_groups, num_frames=num_frames)

        self.tra_1 = ConvBNReLU(64, mid_dim, 3, 1, 1)
        self.siu_1 = ABSIU(mid_dim, siu_groups)
        self.hmu_1 = MGFU(mid_dim, hmu_groups, num_frames=num_frames)
    ...
    def body(self, data):
        ...
        x4 = self.hmu_4(lms + resize_to(x5, tgt_hw=lms.shape[-2:]))

        l, m, s = (self.tra_3(l_trans_feats[2]),self.tra_3(m_trans_feats[2]),self.tra_3(s_trans_feats[2]),)
        lms = self.siu_3(l=l, m=m, s=s)
        x3 = self.hmu_3(lms + resize_to(x5, tgt_hw=lms.shape[-2:])+ resize_to(x4, tgt_hw=lms.shape[-2:]))

        l, m, s = (self.tra_2(l_trans_feats[1]),self.tra_2(m_trans_feats[1]),self.tra_2(s_trans_feats[1]))
        lms = self.siu_2(l=l, m=m, s=s)
        x2 = self.hmu_2(lms + resize_to(x5, tgt_hw=lms.shape[-2:])+ resize_to(x4, tgt_hw=lms.shape[-2:])+ resize_to(x3, tgt_hw=lms.shape[-2:]))


        l, m, s = (self.tra_1(l_trans_feats[0]),self.tra_1(m_trans_feats[0]),self.tra_1(s_trans_feats[0]),)
        lms = self.siu_1(l=l, m=m, s=s)
        x1 = self.hmu_1(lms + resize_to(x5, tgt_hw=lms.shape[-2:])+ resize_to(x4, tgt_hw=lms.shape[-2:])+ resize_to(x3, tgt_hw=lms.shape[-2:])+ resize_to(x2, tgt_hw=lms.shape[-2:]))

        return self.predictor(x1)


class PvtV2B2_MSRNet(_MSRNet_Base):
    def __init__(
        self,
        pretrained=True,
        num_frames=1,
...
class EffB1_MSRNet(_MSRNet_Base):
...
class EffB4_MSRNet(EffB1_MSRNet):
    def set_backbone(self, pretrained):
